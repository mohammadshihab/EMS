...
void ILI9341_DrawChar(char ch, const uint8_t font[], uint16_t X, uint16_t Y, uint16_t color, uint16_t bgcolor)
{
	...
	for (int j=0; j < fHeight; j++)
	{
		for (int i=0; i < fWidth; i++)
		{
			...
			if (( z & b ) != 0x00)
			{
				// HERE YOU CAN DRAW PIXEL JUST ONE BESIDE OTHER TO MAKE THE TEXT BOLD
				ILI9341_DrawPixel(X+i, Y+j, color);
	            ILI9341_DrawPixel(X+i+1, Y+j, color);
				//ILI9341_DrawPixel(X+i+2, Y+j, color);

				ILI9341_DrawPixel(X+i, Y+j+1, color);
				ILI9341_DrawPixel(X+i+1, Y+j+1, color);
				//ILI9341_DrawPixel(X+i+2, Y+j+1, color);
				i=i+1;
			}
		}
	}
...
void ILI9341_DrawNumber(double Nmbr, const uint8_t font[], uint16_t X, uint16_t Y, uint16_t color, uint16_t bgcolor)
{
	char charr[10];
	sprintf(charr,"%.3f",Nmbr);
	ILI9341_DrawText(charr, font, X, Y, color, bgcolor);
}



void ILI9341_DrawNumberF(float Nmbr, const uint8_t font[], uint16_t X, uint16_t Y, uint16_t color, uint16_t bgcolor)
{
    char charr[10];
    sprintf(charr, "%.3f", Nmbr);
    ILI9341_DrawText(charr, font, X, Y, color, bgcolor);
...
void ILI9341_DrawNumberInt(uint16_t Nmbr, const uint8_t font[], uint16_t X, uint16_t Y, uint16_t color, uint16_t bgcolor)
{
	char charr[10];
	sprintf(charr,"%d",Nmbr);
	ILI9341_DrawText(charr, font, X, Y, color, bgcolor);
}
